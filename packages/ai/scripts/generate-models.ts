...
async function generateModels() {
	...
	// Ensure every KnownProvider always has an entry in the output even if all
	// network fetches fail (e.g. self-signed cert in corporate environments).
	// An empty provider object is valid — getModels() returns [] at runtime —
	// and it keeps `keyof typeof MODELS` stable so type-checked callers like
	// getModel("github-copilot", ...) never fail to compile.
