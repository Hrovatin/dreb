...
**Pasted images.** Paste an image straight into the composer (e.g. a screenshot, `Cmd/Ctrl+V`) to attach it to your next message. Each pasted image shows as a **removable thumbnail chip**; you can attach several and remove them individually before sending. Unlike tagged context (which folds into the prompt text), images are **not** stringified — on send they travel as separate **image content parts** over RPC (`ImageContent` = base64 + MIME type) so a vision-capable model sees them directly. An image-only message (no typed text) is allowed, and pasting non-image clipboard content (text/code) is left untouched. The pure clipboard-item detection + data-URL parsing live in the vscode-free `webview/image-paste.ts` (unit-tested); the host maps each attachment to the agent's `ImageContent` before dispatching via `prompt`/`steer`.
...
- **A trusted workspace** — the extension is **not supported in untrusted workspaces**. It declares no [Workspace Trust](https://code.visualstudio.com/docs/editor/workspace-trust) capability, so opening an untrusted folder disables it entirely: no dreb icon in the Activity Bar, no commands in the palette, nothing. If the dreb icon is missing in a project, that folder is almost certainly untrusted — trust it (click **Trust** on the folder prompt, or run **Workspaces: Manage Workspace Trust**) and the extension comes back.
