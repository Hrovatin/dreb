...
- **Read-only Ask mode.** Toggle a safe, reversible read-only session with `/ask on` / `/ask off` (footer shows an `ASK` indicator): edits/writes are disabled, `bash` is limited to an allowlist of read-only commands, and `subagent` delegation is restricted to read-only agent types — ideal for questions, code understanding, and brainstorming without touching your project.
