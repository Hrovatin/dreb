...
- **Read-only Ask mode.** Toggle a safe, reversible read-only session with `/ask on` / `/ask off` (footer shows an `ASK` indicator): edits/writes and the `bash` shell are disabled, exploration runs through the dedicated read-only tools (`read`, `grep`, `find`, `ls`, `search`) plus a typed read-only `git` tool, and `subagent` delegation is restricted to read-only agent types — ideal for questions, code understanding, and brainstorming without touching your project. (The guarantee comes from the capability tool set, not an OS sandbox — there is no filesystem/network confinement; OS-level sandboxing is future work.)
